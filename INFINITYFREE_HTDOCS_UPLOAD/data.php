...
function sendActualEmail($toEmail, $toName, $subject, $htmlBody, $voucherCode = 'WELCOME15', $userId = '', $senderInfo = [], $templateType = 'welcome') {
    ...
    if ($templateType === 'subscribe' || $templateType === 'newsletter') {
    ...
    } elseif ($templateType === 'order' || $templateType === 'tracking') {
        // Order & Live Tracking: ใช้ EmailJS Account ที่ 2 (รหัสล่าสุด: service_3n2rkol / template_pt8dolh / W-8QyJeFcTQcTtoFA)
        if (!empty($emailJsConfig['order_enabled']) && $emailJsConfig['order_enabled'] &&
            ...
            // Fallback: ใช้ account หลัก + template_welcome ไปก่อน
    ...
    if ($activeDriver === 'emailjs') {
        $extraParamsMerged = array_merge(
            is_array($senderInfo) ? $senderInfo : [],
            [
        ...
        $liveResult = EmailJsMailer::send($toEmail, $toName, $subject, $htmlBody, $currentEmailJsConfig, $extraParamsMerged);
    ...
    $inboxType = $templateType ?: 'welcome';
    $inboxText = '';
    if ($templateType === 'tracking') {
        $inboxType = 'tracking';
        $inboxText = '🚐 ความคืบหน้าการจัดส่งน้องแมว: ' . $subject . (!empty($senderInfo['delivery_note']) ? ' (' . $senderInfo['delivery_note'] . ')' : '');
    } elseif ($templateType === 'order') {
        $inboxType = 'order';
        $inboxText = '✅ ยืนยันคำสั่งซื้อ: ' . $subject;
    } elseif ($templateType === 'subscribe' || $templateType === 'newsletter') {
        $inboxType = 'subscribe';
        $inboxText = '📬 ขอบคุณที่ติดตามข่าวสาร Purrfect Cat Club (โค้ดส่วนลด 10%: ' . $voucherCode . ')';
    } else {
        $inboxType = 'welcome';
        $inboxText = 'ขอต้อนรับคุณ ' . $toName . ' สู่คอมมูนิตี้คนรักแมวสายพันธุ์แท้! สิทธิพิเศษต้อนรับสมาชิกใหม่ ลดทันที 15% (โค้ด: ' . $voucherCode . ') สำหรับการรับเลี้ยงน้องแมวตัวแรก พร้อมตรวจสุขภาพและจัดส่งรถตู้แอร์ปรับอุณหภูมิฟรีถึงหน้าบ้าน';
    }
...
/**
 * sendTrackingStatusUpdateEmail — ส่งอีเมลแจ้งเตือนความคืบหน้าการจัดส่งสัตว์เลี้ยง (Live Tracking)
 * ใช้ EmailJS Account ที่ 2 (รหัสล่าสุด: service_3n2rkol / template_pt8dolh / W-8QyJeFcTQcTtoFA)
 *
 * @param array  $orderData     ข้อมูลคำสั่งซื้อจาก data_orders.json
 * @param string $currentStatus vet_check | grooming | transit | delivered
 * @param string $deliveryNote  บันทึกเพิ่มเติมจากแอดมินหรือพี่เลี้ยง
 * @return array ผลลัพธ์การส่ง
 */
function sendTrackingStatusUpdateEmail(array $orderData, string $currentStatus, string $deliveryNote = ''): array {
    $customerName  = $orderData['customer_name']  ?? 'คุณลูกค้า';
    $customerEmail = trim($orderData['customer_email'] ?? '');
    $orderId       = $orderData['order_id']       ?? ($orderData['invoice_id'] ?? 'N/A');
    $trackingId    = $orderData['tracking_id']    ?? ('TRACK-TH-' . strtoupper(substr(md5($orderId), 0, 8)));
    ...
    $statusTitles = [
        'vet_check' => '🩺 ตรวจสุขภาพก่อนเดินทางเรียบร้อย',
        'grooming'  => '🛁 กรูมมิ่ง & เตรียมกล่องเดินทางพร้อมแล้ว',
        'transit'   => '🚐 กำลังออกเดินทางส่งมอบ (In Transit)',
        'delivered' => '🏡 ส่งมอบน้องแมวถึงมือเรียบร้อยแล้ว 🐾'
    ];
    $statusTitle = $statusTitles[$currentStatus] ?? '🚐 อัปเดตสถานะการจัดส่ง';

    require_once __DIR__ . '/email_tracking_update.php';
    $rendered = renderTrackingStatusEmail(
        $customerName, $customerEmail, $orderId, $trackingId,
        $currentStatus, $orderData, $deliveryNote
    );

    $subject = "🚐 [อัปเดตการจัดส่ง #{$trackingId}] {$statusTitle} - Purrfect Shop 🐾";

    $res = sendActualEmail($customerEmail, $customerName, $subject, $rendered, '', '', [
