...
class EmailJsMailer {
    ...
    public static function send($toEmail, $toName, $subject, $htmlBody, $config, $extraParams = []) {
        ...
        $conciseText = mb_strimwidth(strip_tags($htmlBody), 0, 1200, '...');

        // Cap HTML body to ~50KB to prevent EmailJS timeout on large order emails
        $safeHtmlBody = (strlen($htmlBody) > 51200)
            ? mb_strimwidth($htmlBody, 0, 51200, '') . '</td></tr></table></body></html>'
            : $htmlBody;
        ...
        $templateParams = array_merge([
            ...
            'recipient' => $toEmail,
            // Space-tolerant aliases (in case user wrote {{ to_email }} in template)
            ' to_email ' => $toEmail,
            'to_email ' => $toEmail,
            ' to_email' => $toEmail,
            ' email ' => $toEmail,
